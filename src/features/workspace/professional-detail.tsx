...
import { CONNECT_STATUS_LABEL, CONNECT_STATUS_TONE, connectStatusOf } from "@/lib/stripe/capabilities";
...
export async function ProfessionalDetail({ professionalId }: { professionalId: string }) {
  ...
  const [{ count: clientCount }, { count: activeClientCount }, { data: paymentAccount }] =
    await Promise.all([
      supabase
        .from("clients")
  ...
  const connectStatus = connectStatusOf(paymentAccount);
    ...
    <>
      ...
      <Card className="mb-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold text-ink-900">Pagamento online</h2>
            <p className="mt-0.5 text-xs text-ink-500">
              Conta Stripe onde este profissional recebe as cobrancas dos clientes dele.
            </p>
          </div>
          <Badge tone={CONNECT_STATUS_TONE[connectStatus]}>
            {CONNECT_STATUS_LABEL[connectStatus]}
          </Badge>
        </div>

        {/* A comissao existe mesmo sem conta conectada: assim ja fica valendo
            quando o profissional terminar o cadastro. */}
        <PlatformFeeForm
          userId={professional.id}
          current={paymentAccount?.platform_fee_percent ?? DEFAULT_PLATFORM_FEE_PERCENT}
        />
        <p className="mt-2 text-xs text-ink-500">
          Retido de cada cobranca paga por este profissional, alem da taxa da Stripe.
        </p>
      </Card>
